...
from fireworks_helpers import FireworksBenchmark, FireworksConfig, StreamingStats
...
class FireworksBenchmarkService:
    ...
    async def run_single_benchmark(
        ...
        try:
            ...
            successful_results = [r for r in individual_results if r.get('tokens', 0) > 0]
            error_results = [r for r in individual_results if 'error' in r]

            # Calculate peak TPS
            peak_tps = max([r.get('tps', 0) for r in successful_results]) if successful_results else 0

            # Calculate completion lengths
            completion_lengths = [
                len(r.get('completion_text', ''))
            ...
            avg_tokens_per_request = (
                sum(r.get('tokens', 0) for r in successful_results) / len(successful_results)
    ...
    async def run_comparison_benchmark(
        ...
        if progress_callback:
            progress_callback("starting_comparison", {
                "models": [self.config.get_model(key)['name'] for key in model_keys],
        ...
        if progress_callback:
            progress_callback("comparison_completed", {
...
class BenchmarkReporter:
    ...
    @staticmethod
    def generate_comparison_report(results: Dict[str, BenchmarkResult]) -> Dict[str, Any]:
        """Generate a comparison report from multiple benchmark results"""
        if not results:
            return {"error": "No results to compare"}

        # Find best performers
        best_aggregate_tps = max(results.values(), key=lambda r: r.aggregate_tokens_per_second)
    ...
    def export_to_csv(results: Dict[str, BenchmarkResult], filepath: str) -> None:
        ...
        with open(filepath, 'w', newline='') as f:
            ...
            # Header
            writer.writerow([
                'Model', 'Model_ID', 'Concurrency', 'Total_Requests',
                'Successful_Requests', 'Success_Rate_%', 'Total_Time_s',
                'Avg_TPS', 'Aggregate_TPS', 'Peak_TPS', 'Avg_TTFT_ms',
                'Total_Tokens', 'Avg_Tokens_Per_Request', 'Error_Rate_%'
            ])
